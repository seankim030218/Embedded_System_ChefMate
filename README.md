...
>
**📹 시연 영상 바로보기 →** [demo/1번.mp4](demo/1번.mp4)
...
| HX711 보정값 관리 | 코드 내 상수(`CALIBRATION_FACTOR = -7050.0`) X하드코딩X | EEPROM 저장 + 런타임 보정 루틴 | 발표 일정 내 구현 범위를 좁혀 핵심 기능(계량+UI)에 집중 | 로드셀 교체/온도 변화 시 재컴파일 필요 → 재현성 저하 | 알려진 무게의 물체로 표시값-실측값 오차 확인 |
...
`SPI_Slave.ino`의 `CALIBRATION_FACTOR = -7050.0`은 **현재 사용 중인 로드셀에 맞춰 X하드코딩X된 값**입니다.
...
| `#3 fix: HX711 보정 루틴 추가 (X하드코딩X → 절차화)` | 트러블슈팅·검증 능력 | ⬜ 예정 |
